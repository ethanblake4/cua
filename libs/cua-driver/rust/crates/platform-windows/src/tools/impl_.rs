...
impl Tool for GetWindowStateTool {
    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        let window_bounds = match tokio::task::spawn_blocking(move || {
            exact_target_window_bounds(pid, hwnd)
        })
        .await
        {
            Ok(Ok(bounds)) => bounds,
            ...
            Err(error) => {
                return ToolResult::error(format!("Error: window bounds task panic: {error}"))
            }
        };
        ...
        match result {
            Ok((tree_opt, screenshot_opt, screenshot_err, window_bounds, tree_error)) => {
